...
import EventosPage from './eventos_page';

const Eventos = ({events,pages,page}) => {
    ...
    const update = (event, value) => {
        let path = "/ayt/eventos/page"+value;
        if(value===1){path = "/ayt/eventos"}
        navigate(path);
    };
    ...
    function divideEvents(){
        let inicio = 6*(page-1);
        let fin = inicio+6;
        if(fin>events.length){
            fin = events.length;
        }
        setSubevents(events.slice(inicio,fin));
    }
