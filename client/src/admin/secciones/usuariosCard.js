...
const UsuariosCard = ({onChange, card}) => {
    const [details, setDetails] = useState({username: card.username, pass: card.pass, nombre:card.nombre, apellidos:card.apellidos, dni:card.dni, fechaNac:card.fechaNac, sexo:card.sexo, rol:card.rol});

    const [date, setDate] = React.useState(dayjs(new Date(card.fechaNac) - 1));
    ...
    return(
        <div>
            <Card elevation={12} sx={{minWidth:500}}>
                <CardHeader
                    avatar={
                        <Avatar sx={{ bgcolor: "#03a9f4" }} variant="rounded">
                            <AccountBoxIcon />
                        </Avatar>
                    }
                    subheader={"Usuario "+card.rol}
                />
                <CardHeader
                    avatar={
                        <Avatar sx={{ bgcolor: "#03a9f4" }} variant="rounded">
